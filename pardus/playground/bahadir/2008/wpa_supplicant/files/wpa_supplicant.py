...
def checkServiceState(serviceName):
    bus = dbus.SystemBus()
    obj = bus.get_object("tr.org.pardus.comar", "/package/%s" % serviceName, introspect=False)
    iface = dbus.Interface(obj, "tr.org.pardus.comar.System.Service")
    _type, _desc, _state = iface.info()
    return _state in ("on", "started")
...
def startWpaService():
    if not isWpaServiceActive():
        bus = dbus.SystemBus()
        obj = bus.get_object("tr.org.pardus.comar", "/package/wpa_supplicant", introspect=False)
        iface = dbus.Interface(obj, "tr.org.pardus.comar.System.Service")
        iface.start()
    return True
    ...
    def authenticate(self, username, password, timeout = 15):
        self.network.setNetwork(
            {"ssid": dbus.String(self.ssid, variant_level=1),
             "key_mgmt": dbus.String("IEEE8021X", variant_level=1),
             "eap": dbus.String("PEAP", variant_level=1),
