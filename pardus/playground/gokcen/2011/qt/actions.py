...
def build():
    # Use new Qt libraries if possible
    shelltools.export("LD_LIBRARY_PATH", "%s/%s/lib" % (get.workDIR(), WorkDir))

    autotools.make()

    # Sometimes doc. generation fails since qdoc3 binary of new qt tries to use old Qt libraries from system. This is a workaround for that
    autotools.make("docs")
