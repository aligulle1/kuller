...
def exportFlags():
    ...
    shelltools.export("CC", "%s-gcc" % get.HOST())
    shelltools.export("CXX", "%s-g++" % get.HOST())
...
def setup():
    ...
    cmd = '../configure \
           ...
           --with-bugurl=http://bugs.pardus.org.tr' % (get.HOST(), get.HOST(), get.HOST(), opt_arch, opt_unwind, opt_multilib)
    ...
    if get.ARCH() == "arm":
        cmd = "sb2 %s --with-gxx-include-dir=%s/usr/include/c++" % (cmd, get.sysroot())
    else:
        cmd += "--with-gxx-include-dir=/usr/include/c++"
