...
def setup():
    shelltools.export("FFI_CFLAGS", "-I%(RootDir)s/usr/lib/libffi-3.0.8/include" % autotools.environment)
    #pisitools.dosed("giscanner/Makefile.in", "py_compile = .*", "py_compile = /bin/true")
    autotools.autoreconf("-fiv")
    #pisitools.dosed("gir/Makefile.in", r"(^PYTHON_INCLUDES\s*=).*", "\\1 -I%(RootDir)s/usr/include/python2.6" % autotools.environment)
