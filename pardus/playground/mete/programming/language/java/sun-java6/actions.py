...
def install():
    pisitools.dodir("/opt")
    shelltools.system("./construct.sh . %s/opt/sun-jdk6 %s/opt/sun-jre6"% (get.installDIR(),get.installDIR()))

    # Install mozilla plugin
    pisitools.dodir("/usr/lib/nsbrowser/plugins")
    pisitools.dosym("/opt/sun-jre6/plugin/i386/ns7/libjavaplugin_oji.so", "/usr/lib/nsbrowser/plugins/javaplugin.so")

    for doc in ["COPYRIGHT", "LICENSE", "README.html", "README_ja.html", "README_zh_CN.html", "THIRDPARTYLICENSEREADME.txt"]:
        file = "%s/opt/sun-jdk6/%s" % (get.installDIR(),doc)
