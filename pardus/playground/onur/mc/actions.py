#!/usr/bin/python
# -*- coding: utf-8 -*-
#
# Copyright 2005-2009 TUBITAK/UEKAE
# Licensed under the GNU General Public License, version 2.
# See the file http://www.gnu.org/licenses/old-licenses/gpl-2.0.txt

from pisi.actionsapi import autotools
from pisi.actionsapi import pisitools
from pisi.actionsapi import shelltools
from pisi.actionsapi import get

# cflags = "-DUTF8=1 -D_FILE_OFFSET_BITS=64 -D_LARGEFILE_SOURCE %s -fgnu89-inline -I/usr/include/gssapi" % get.CFLAGS()
# cflags = "-DUTF8=1 -D_FILE_OFFSET_BITS=64 -D_LARGEFILE_SOURCE %s" % get.CFLAGS()
cflags = "-D_FILE_OFFSET_BITS=64 -D_LARGEFILE_SOURCE %s" % get.CFLAGS()

def setup():
    shelltools.export("CFLAGS", cflags)
    #shelltools.move("po/no.po",  "po/nb.po")
    # shelltools.move("po/no.gmo", "po/nb.gmo")
    # shelltools.unlink("po/tr.gmo")

    # Make sure we don't need cvs for autopoint
    shelltools.export("AUTOPOINT", "/bin/true")
    autotools.autoreconf("-vfi")
    autotools.configure("--disable-rpath \
                         --with-screen=slang \
                         --with-gpm-mouse \
                         --with-vfs \
                         --with-ext2undel \
                         --with-edit \
                         --with-x=yes \
                         --enable-charset \
                         --enable-nls \
                         --enable-vfs-smb \
                         --enable-vfs-mcfs \
                         --with-samba \
                         --with-configdir=/etc/samba \
                         --with-codepagedir=/var/lib/samba/codepages \
                         --with-privatedir=/etc/samba/private")
                         # --with-included-gettext \

def build():
    shelltools.export("CFLAGS", cflags)
    autotools.make()

    #shelltools.cd("po")
    #autotools.make("tr.gmo")

def install():
    autotools.rawInstall('DESTDIR="%s"' % get.installDIR())

    # pisitools.dodoc("AUTHORS", "ChangeLog", "MAINTAINERS", "FAQ", "HACKING", "README*", "TODO")
    pisitools.dodoc("AUTHORS", "ChangeLog", "doc/FAQ", "doc/COPYING", "doc/NEWS", "doc/README")

    # Do not mess with glibc files
    # pisitools.remove("/usr/share/locale/locale.alias")

    # Do not carry empty dirs
    #pisitools.removeDir("/usr/sbin")
    #pisitools.removeDir("/usr/share/man/man8")
    #pisitools.removeDir("/usr/share/man/sr/man8")

    for i in ["mc.sh", "mc.csh"]:
        pisitools.insinto("/etc/profile.d/", "contrib/%s" % i)


