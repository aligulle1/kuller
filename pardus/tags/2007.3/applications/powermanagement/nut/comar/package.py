...
def postInstall():
    for _file in PUBLIC_FILES:
        f = "/etc/nut/%s" % _file
        os.chmod(f, 0644)
        os.system("/usr/bin/chown %s:%s %s" % ("root", "root", f))

    for _file in PRIVATE_FILES:
        f = "/etc/nut/%s" % _file
        os.chmod(f, 0640)
        os.system("/usr/bin/chown %s:%s %s" % (PRIVATE_USER, PRIVATE_GROUP, f))

    os.chmod(STATE_DIR, 0770)
    os.system("/usr/bin/chown %s:%s %s" % (PRIVATE_USER, PRIVATE_GROUP, STATE_DIR))
