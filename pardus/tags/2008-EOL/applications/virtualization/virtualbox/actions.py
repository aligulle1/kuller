...
def install():
    ...
    relativeInstallDir = "../lib/virtualbox"
    for link in apps:
    ...
    # Add a symlink for KDE 3
    pisitools.dosym("/etc/xdg/autostart/vboxclient.desktop", "%s/share/autostart/vboxclient.desktop" % get.kdeDIR())

    pisitools.insinto("/usr/bin", "src/VBox/Additions/x11/Installer/98vboxadd-xclient", "VBoxClient-all")
    shelltools.chmod("%s/usr/bin/VBoxClient-all" % get.installDIR(), 0755)
