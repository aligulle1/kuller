...
def setup():
    ...
    # Change C/XXFLAGS
    pisitools.dosed("Makefile", "^CFLAGS.*\\$\\(DEFINES\\)", "CFLAGS   = %s $(DEFINES)" % get.CFLAGS())
    pisitools.dosed("Makefile", "^CXXFLAGS.*\\$\\(DEFINES\\)", "CXXFLAGS   = %s $(DEFINES)" % get.CXXFLAGS())
...
def install():
    ...
    pisitools.dodoc("ChangeLog", "LICENSE*", "NEWS", "README", "OPENSOURCE-NOTICE.TXT")
