...
def setup():
    pisitools.dosed("mkspecs/common/g++.conf", "^QMAKE_CFLAGS_RELEASE.*", "QMAKE_CFLAGS_RELEASE\t+= %s" % get.CXXFLAGS())
    ...
    autotools.rawConfigure("-no-pch \
                            ...
                            -plugin-sql-ibase \
                            -I/usr/include/mysql/ \
                            -I/usr/include/firebird/include/ \
...
def install():
    ...
    # TODO: Use this when Pisi stops raising exceptions about unnecessary doseds.
    ...
    for f in ("QtAssistantClient.pc", "QtDesignerComponents.pc", "QtDesigner.pc", "QtUiTools.pc"):
        pisitools.dosed("%s/usr/lib/pkgconfig/%s" % (get.installDIR(), f), "%s/qt-x11-opensource-src-%s" % (get.workDIR(),get.srcVERSION()),"/usr/qt/4")
