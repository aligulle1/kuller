...
def install():
    ...
    shelltools.system("/sbin/mkinitramfs --full --net kernel=%s "
                      "--root-dir=%s/opt/ptsp --output=%s/tftpboot/pts/%s-ptsp/" %
                      (suffix, get.installDIR(), get.installDIR(), suffix))

    pisitools.remove("/tftpboot/pts/latest-ptsp")
    pisitools.remove("/tftpboot/pts/%s-ptsp/latestinitramfs" % suffix)
    pisitools.remove("/tftpboot/pts/%s-ptsp/latestkernel" % suffix)
