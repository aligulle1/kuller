        ...
        if key in ["root", "rootnoverify"]:
            ...
            try:
                ...
                if "root=" in options:
                    ...
                    # Check whether this is a Pardus kernel
                    if os_entry.has_key("root"):
                        os_entry["is_pardus_kernel"] = str((getRoot() == os_entry["root"]))
                    elif os_entry.has_key("uuid"):
                        os_entry["is_pardus_kernel"] = str((getRoot() == getDeviceByUUID(os_entry["uuid"])))
                    else:
                        os_entry["is_pardus_kernel"] = ""
...
def makeGrubEntry(title, os_type, root=None, kernel=None, initrd=None, options=None, use_uuid_as_root=False):
    ...
    # May be an UUID
    may_be_uuid = getDeviceByUUID(root.lower())
    if may_be_uuid:
        root = may_be_uuid
    ...
    if os_type == "windows":
        ...
        if use_uuid_as_root:
            # Write uuid string instead of root
            uuid = getUUIDByDevice(root)
            if uuid:
                entry.setCommand("uuid", uuid)
            else:
                # Can't find uuid, use root
...
def updateKernelEntry(version, root):
    ...
    if new_version not in versions:
        ...
        new_entry = makeGrubEntry(title, os_type, root, kernel, initrd, boot_parameters, use_uuid_as_root=use_uuid)
